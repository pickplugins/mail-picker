...
function mail_picker_subscriber_source_check()
{
    ...
    if ($campaign_query->have_posts()) :
        ...
        while ($campaign_query->have_posts()) :
            ...
            $post_id = get_the_ID();
            $process_status = get_post_meta($post_id, 'process_status', true);

            error_log("process_status: " . $process_status);
...
function mail_picker_subscriber_source_check_registered_users($post_id)
{
    ...
    $status_after_confirm     = get_post_meta($post_id, 'status_after_confirm', true);
    ...
    foreach ($users as $user) {
        ...
        $formFieldData = array();
        $formFieldData['email'] = $user_email;
        $formFieldData['first_name'] = $display_name;
        $formFieldData['status'] = $status;
...
add_action('mail_picker_subscriber_source_check_csv', 'mail_picker_subscriber_source_check_csv');

function mail_picker_subscriber_source_check_csv($post_id)
{

    error_log("mail_picker_subscriber_source_check_csv");
    ...
    while (($line = fgetcsv($file)) !== FALSE) {
        ...
        $first_name = isset($line[0]) ? $line[0] : '';
        $last_name = isset($line[1]) ? $line[1] : '';
        $email = isset($line[2]) ? $line[2] : '';
        $address_line_1 = isset($line[3]) ? $line[3] : '';
        $postal_code = isset($line[4]) ? $line[4] : '';
        $city = isset($line[5]) ? $line[5] : '';
        $state = isset($line[6]) ? $line[6] : '';
        $country = isset($line[7]) ? $line[7] : '';
        $phone = isset($line[8]) ? $line[8] : '';
    ...
    foreach ($users as $user) {
        ...
        //update_post_meta($user_email, 'subscriber_source_check_' . $post_id, 'done');

        //update_user_meta($user_id, 'subscriber_source_check_' . $post_id, 'done');
    }

    update_post_meta($post_id, 'process_status', "complete");
}
...
function mail_picker_subscriber_source_check_comments($post_id)
{
    ...
    foreach ($comments as $comment) {
        ...
        $formFieldData = array();
        $formFieldData['email'] = $author_email;
...
function mail_picker_subscriber_source_check_woo_orders($post_id)
{
    ...
    if ($wp_query->have_posts()) :
        while ($wp_query->have_posts()) : $wp_query->the_post();
            ...
            $formFieldData = array();
            $formFieldData['email'] = $billing_email;
                ...
                if ($status == 'success') {
                    ...
                    if ($send_confirmation_mail == 'yes') {
                        ...
                        $vars = array(
                            ...
                            '{email}' => esc_url_raw($billing_email),
...
function mail_picker_subscriber_source_check_flamingo_inbound($post_id)
{
    ...
    if ($wp_query->have_posts()) :
        while ($wp_query->have_posts()) : $wp_query->the_post();
            ...
            $formFieldData = array();
            $formFieldData['email'] = $_from_email;
...
if (!function_exists('mail_picker_wpcf7_submit')) {
    function mail_picker_wpcf7_submit($contact_form)
    {
        ...
        $status = (isset($cf7['status']) && !empty($cf7['status'])) ? $cf7['status'] : 'pending';
        ...
        $formFieldData = array();
        $formFieldData['email'] = $email_value;
        ...
        //        ) );
        //
        //        update_post_meta( $subscriber_id, 'email', $email_value );
...
function mail_picker_wpforms_process_complete($fields, $entry, $form_data, $entry_id)
{
    ...
    $status = (isset($wpforms['status']) && !empty($wpforms['status'])) ? $wpforms['status'] : 'pending';
    ...
    //update_post_meta( $subscriber_id, 'email', $subscriber_id );
...
function mail_picker_frm_after_create_entry($entry_id, $form_id)
{
    ...
    $status = (isset($formidable['status']) && !empty($formidable['status'])) ? $formidable['status'] : 'pending';
...
function mail_picker_caldera_custom_form_submit_before_set_fields($entry, $form_id, $field_data_array)
{
    ...
    $status = (isset($caldera['status']) && !empty($caldera['status'])) ? $caldera['status'] : 'pending';
...
function mail_picker_weforms_entry_submission($entry_id, $form_id, $page_id, $form_settings)
{
    ...
    $status = (isset($weforms['status']) && !empty($weforms['status'])) ? $weforms['status'] : 'pending';
...
function kaliforms_before_form_process($data)
{
    ...
    $status = (isset($kaliforms['status']) && !empty($kaliforms['status'])) ? $kaliforms['status'] : 'pending';
...
function ig_es_add_subscriber_data($data)
{
    ...
    $status = (isset($email_subscribers['status']) && !empty($email_subscribers['status'])) ? $email_subscribers['status'] : 'pending';
