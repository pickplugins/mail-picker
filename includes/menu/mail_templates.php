...
$mail_picker_settings = get_option('mail_picker_license');
$license_key = isset($mail_picker_settings['license_key']) ? $mail_picker_settings['license_key'] : '';

$max_num_pages = 0;

wp_enqueue_script('mail_picker_layouts');


?>
<div class="wrap">
    <h2><?php _e('Mail Picker - Email Templates Library', 'mail-picker'); ?></h2>

    <div class="wpblockhub-search">

        <div class="wp-filter">
            <ul class="filter-links">
                <li class=""><a href="<?php echo esc_url_raw($_SERVER['REQUEST_URI']); ?>&tabs=latest" class="<?php if($tabs == 'latest') echo 'current'; ?>" aria-current="page"><?php _e('Latest', 'mail-picker'); ?></a> </li>
                <li class=""><a href="<?php echo esc_url_raw($_SERVER['REQUEST_URI']); ?>&tabs=free" class="<?php if($tabs == 'free') echo 'current'; ?>" aria-current="page"><?php _e('Free', 'mail-picker'); ?></a> </li>
                <li class=""><a href="<?php echo esc_url_raw($_SERVER['REQUEST_URI']); ?>&tabs=pro" class="<?php if($tabs == 'pro') echo 'current'; ?>" aria-current="page"><?php _e('Premium', 'mail-picker'); ?></a> </li>
            </ul>
        ...
        $api_params = array(
            'mail_picker_remote_action' => 'layoutSearch',
            'keyword' => $keyword,
            'paged' => $paged,
            'tabs' => $tabs,
        );
        ...
        $response = wp_remote_get(add_query_arg($api_params, mail_picker_server_url), array('timeout' => 20, 'sslverify' => false));
        ...
        if (is_wp_error($response)){
            ...
            <div class="return-empty">
                <ul>
                    <li><?php echo __("Unexpected Error! The query returned with an error.", 'mail-picker'); ?></li>
                    <li><?php echo __("Make sure your internet connection is up.", 'mail-picker'); ?></li>
                </ul>
            </div>
            <?php
        ...
        <div class="block-list-items">
            ...
            if(!empty($post_data)):
                ...
                foreach ($post_data as $item_index=>$item):
                    ...
                    $block_title        = isset($item->title) ? $item->title : __('No title', 'mail-picker');
